...
def line_intersection(x1, y1, x2, y2, x3, y3, x4, y4):
    ...
    if ((x1 >= x3 - 5 or x1 >= x3 + 5) and (x1 <= x4 + 5 or x1 <= x4 - 5)
            and (y3 + 8 >= min(y1, y2) or y3 - 5 >= min(y1, y2))
            and y3 <= max(y1, y2) + 5):
        return x1, y3
...
def extractText(img):
    ...
    for cnt in contours:
        ...
        # print(im2.shape)
        if x < 2 or y < 2 or (
                x + w >= im2.shape[1] - 1 and y + h >= im2.shape[0] - 1
        ) or w >= im2.shape[1] - 1 or h >= im2.shape[0] - 1:
            continue
        if x < mx:
            mx = x
        if y < my:
